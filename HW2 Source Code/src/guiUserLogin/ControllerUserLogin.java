	...
	*/
	...
	protected static void doLogin(Stage ts) {
		...
		// System.out.println("*** Username is valid");
     	//fetch the actual password and the one time password
     	String actualPassword = theDatabase.getCurrentPassword();
     	String otp = theDatabase.getOneTimePassword(username);
     	//if one time password is used, force a password reset
     	if (otp != null && password != null && password.equals(otp) && actualPassword.equals(otp)) {
     		ViewUserLogin.alertOTP.setTitle("Password Reset Required");
     		ViewUserLogin.alertOTP.setHeaderText("One-Time Password Used");
     		ViewUserLogin.alertOTP.setContentText("You must reset your password before logging in.");
     		ViewUserLogin.alertOTP.showAndWait();
    	   
    	    try {
    	    	//launch password reset GUI
            PasswordEvaluationGUITestbed pet =
                   new PasswordEvaluationGUITestbed();

            Stage passwordStage = new Stage();
            pet.start(passwordStage);
            // Wait until the popup closes, update database and user object
            passwordStage.setOnHidden(ev -> {

                String newPassword = Model.passwordInput;

                if (newPassword != null && !newPassword.isEmpty()) {
                    theDatabase.updateNewPassword(user.getUserName(), newPassword);
                    user.setPassword(newPassword);
                    theDatabase.clearOneTimePassword(username);
                    ViewUserLogin.text_Username.clear();
    	    ...
    	    return;  // Stop further login checks
    	}
